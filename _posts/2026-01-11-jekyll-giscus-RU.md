...
- GitHub является провайдером SSO, и у большинства читающих блог на GitHub Pages скорее всего есть учетная запись GitHub. Для того чтобы оставить комментарий требуется учетная запись, но при этом не требуется регистрация в сторонних сервисах.
...
Минусом, на мой взгляд, является то что это **вендорлок на GitHub** (которого в принципе сложно избежать используя сторонние сервисы комментариев, можно разве что написать свой бекенд или использовать что-нибудь развернутое у себя)
...
**utterances** приятней тем что обладает более простым и лаконичным интерфейсом от issues. Это стабильный и безопасный проект, проверенный годами, но само использованием issues в качестве комментариев к записям сложно назвать использованием по назначению.
...
![utterances vs giscus comparison](/data/images/posts/giscus/comparison-2.png)
...
![utterances vs giscus comparison](/data/images/posts/giscus/comparison-1.png)

Я решил установить **giscus** т.к. discussions в качестве бекенда мне все же кажется более подходящим решением.

### Подключение

Просто следуйте [инструкции](https://giscus.app/ru) и не забудьте включить Discussions в вашем репозитории. Я рекомендую вынести блок с комментариями в отдельный файл. giscus так же позволяет настроить язык интерфейса, и я настроит его таким образом чтобы он совпадал с языком статьи.
...
На мой взгляд, это один из тех проектов, которые делают ровно то чего от них ож.
